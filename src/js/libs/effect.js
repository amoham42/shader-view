...
export function bufferID_to_assetID( id )
{
...
export function assetID_to_bufferID( id )
{
...
export function assetID_to_cubemapBuferID( id )
{
    if( id==='4dX3Rr' ) return 0;
    return -1;
}
export function cubamepBufferID_to_assetID( id )
{
...
export class EffectPass {
    ...
    NewShader_Sound(shaderCode, commonShaderCodes) {
        let vsSource = null;
    ...
    NewShader_Image(shaderCode, commonShaderCodes) {
        this.mSupportsVR = false;
    ...
    NewShader_Cubemap(shaderCode, commonShaderCodes) {
        let vsSource = null;
        if (this.mIs20)
            vsSource = "layout(location = 0) in vec2 pos; void main() { gl_Position = vec4(pos.xy,0.0,1.0); }";
    ...
    NewTexture(wa, slot, url, buffers, cubeBuffers, keyboard) {
        ...
        if (url === null || url.mType === null) {
            ...
            let returnValue = {
                mFailed: false, mNeedsShaderCompile: (this.mInputs[slot] === null) || (
    ...
    Paint_Image(vrData, wa, d, time, dtime, fps, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, buffers, cubeBuffers, keyboard) {
    ...
    Paint_Cubemap(vrData, wa, d, time, dtime, fps, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, buffers, cubeBuffers, keyboard, face) {
...
export class Effect {
  ...
  me.mCreated = true;
    ...
    GetTexture(passid, slot) { return this.mPasses[passid].GetTexture(slot); }
    NewTexture(passid, slot, url) { return this.mPasses[passid].NewTexture(this.mAudioContext, slot, url, this.mBuffers, this.mCubeBuffers, this.mKeyboard); }
    SetOutputs(passid, slot, url) { this.mPasses[passid].SetOutputs(slot, url); }
    SetOutputsByBufferID(passid, slot, id) { this.mPasses[passid].SetOutputsByBufferID(slot, id); }
    GetAcceptsLinear(passid, slot) { return this.mPasses[passid].GetAcceptsLinear(slot); }
    GetAcceptsMipmapping(passid, slot) { return this.mPasses[passid].GetAcceptsMipmapping(slot); }
    GetAcceptsWrapRepeat(passid, slot) { return this.mPasses[passid].GetAcceptsWrapRepeat(slot); }
    GetAcceptsVFlip(passid, slot) { return this.mPasses[passid].GetAcceptsVFlip(slot); }
    SetSamplerFilter(passid, slot, str) { this.mPasses[passid].SetSamplerFilter(slot, str, this.mBuffers, this.mCubeBuffers); }
    GetTranslatedShaderSource(passid) { return this.mPasses[passid].GetTranslatedShaderSource(); }
    GetSamplerFilter(passid, slot) { return this.mPasses[passid].GetSamplerFilter(slot); }
    SetSamplerWrap(passid, slot, str) { this.mPasses[passid].SetSamplerWrap(slot, str, this.mBuffers); }
    GetSamplerWrap(passid, slot) { return this.mPasses[passid].GetSamplerWrap(slot); }
    SetSamplerVFlip(passid, slot, str) { this.mPasses[passid].SetSamplerVFlip(slot, str); }
    GetSamplerVFlip(passid, slot) { return this.mPasses[passid].GetSamplerVFlip(slot); }
    ...
    GetPassType(id) { return this.mPasses[id].mType; }
    GetPassName(id) { return this.mPasses[id].mName; }
    GetCode(id) { return this.mPasses[id].mSource; }
    SetCode(id, source) { this.mPasses[id].SetCode(source); }
    GetError(id) { return this.mPasses[id].mError; }
    GetErrorStr(id) { return this.mPasses[id].mErrorStr; }
    GetErrorGlobal() {
        for (let i = 0; i < this.mPasses.length; i++) {
    ...
    CompileSome(passes, preventCache, onResolve) {
        let me = this;
    ...
    AddPass(passType, passName, onResolve) {
        let shaderStr = null;
